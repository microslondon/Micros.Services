...
namespace Imagingservice.Tests
{
    public class ImageProcessorTests
    {
        ...
        [Fact()]
        public void FailNonImageFile()
        {
            const int numberOfFiles = 0;
            var configuration = GetClientConfiguration(0, ImageResizerImageProcessorType);
            var successfullyProcessedFilesCount = new FileProcessor(configuration).Process();
            Assert.Equal(0, successfullyProcessedFilesCount);
        }


        [Fact()]
        public void FailPartialFile ()
        {
